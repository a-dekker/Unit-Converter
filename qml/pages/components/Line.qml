...
Item {
    ...
    property real objW: isHorizontal
                        && isVertical ? canvas.sLine2Y : isHorizontal
                                        && !isVertical ? Math.abs(
                                                             canvas.sLine1Y - canvas.sLine2Y) : 0.0
    property real objH: isHorizontal
                        && isVertical ? canvas.sLine1X : !isHorizontal
                                        && isVertical ? Math.abs(
                                                            canvas.sLine1X - canvas.sLine2X) : 0.0
    ...
    function findLine(x, y) {
        ...
        var lineArray = [calculatePointLineDistance(
                             x, y, 1) < 50, calculatePointLineDistance(x, y,
                                                                       2) < 50]
    ...
    function calculatePointLineDistance(x, y, lineNum) {
        ...
        case 1:
            var d1 = Math.abs(
                        (canvas.sLine1X - canvas.eLine1X) * (canvas.sLine1Y - y)
                        - (canvas.sLine1X - x) * (canvas.sLine1Y - canvas.eLine1Y))
                    / Math.sqrt(Math.pow(canvas.sLine1X - canvas.eLine1X,
                                         2) + Math.pow(
                                    canvas.sLine1Y - canvas.eLine1Y, 2))
            return d1
        case 2:
            var d2 = Math.abs(
                        (canvas.sLine2X - canvas.eLine2X) * (canvas.sLine2Y - y)
                        - (canvas.sLine2X - x) * (canvas.sLine2Y - canvas.eLine2Y))
                    / Math.sqrt(Math.pow(canvas.sLine2X - canvas.eLine2X,
                                         2) + Math.pow(
                                    canvas.sLine2Y - canvas.eLine2Y, 2))
            return d2
        }
    }
}
